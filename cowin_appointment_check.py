...
def send_email(email_content,place_name,to_email=[],age=18):
	...
	try:
		if(len(to_email)>0):	
			intro = 'Vaccine for age <strong>{}</strong>+ in <strong>{}</strong> available at {} pincode(s) as on {} {}'.format(age,place_name,len(email_content),curr_time,curr_date)
			...
			for key in sorted_keys:
				...
				if(len(center_list)>0):
					for center in center_list:
						...
						row = '<tr>{}{}{}{}</tr>'.format(col1,col2,col3,col4)
						table_contents += row
			...
			if(apply_filter(dist_payload['district_id'],subscriber_idx,clinic_idx)):
				...
				for sessions in clinics['sessions']:
					if(sessions['min_age_limit'] <= int(group['age']) and sessions['available_capacity']>0):
						clinic['date_slots'].append(sessions['date']+'('+str(sessions['available_capacity'])+')')
				...
				if(len(clinic['date_slots'])>0):temp[clinics['pincode']].append(clinic)
	except Exception as e:
		print("Error ",e)
	finally:
		return temp
...
for s_idx, group in enumerate(subscribers):
	...
	print('Last status fetched for '+place_name+' at '+str(curr_time)+' '+str(curr_date)+' for age '+str(group['age'])+', no. of centers:',len(dist_res))
	if(len(dist_res)>0):send_email(dist_res,place_name,group['to_email'],group['age'])